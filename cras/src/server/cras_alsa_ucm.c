			    ...
			    enum CRAS_STREAM_DIRECTION dir)
{
	...
	if (!section_names) {
		if (dir == CRAS_STREAM_OUTPUT) {
			section_names = ucm_get_devices_for_var(mgr, playback_mixer_elem_var, mixer, dir);
		} else if (dir == CRAS_STREAM_INPUT) {
			section_names = ucm_get_devices_for_var(mgr, capture_mixer_elem_var, mixer, dir);
		}
	}
...
inline const char *ucm_get_playback_mixer_elem_name_for_dev(struct cras_use_case_mgr *mgr,
					      const char *dev)
{
	return ucm_get_value_for_dev(mgr, playback_mixer_elem_var, dev);
}

inline const char *ucm_get_capture_mixer_elem_for_dev(struct cras_use_case_mgr *mgr,
					      const char *dev)
{
